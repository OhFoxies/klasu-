...
class BackgroundTasks:
    ...
    async def background_tasks(self):
        ...
        schedule.daily(dt.time(hour=7, minute=0), self.start_new_tasks, args=(self.lucky_numbers_sender_between_callbacks,))
        logs_.log("Background task lucky numbers sender (Every day at 7:00) has been loaded")

        schedule.hourly(dt.time(minute=0, second=0), self.start_new_tasks, args=(self.lucky_numbers_checker_between_callbacks,))
        logs_.log("Background task lucky numbers checker (Every hour) has been loaded")

        schedule.daily(dt.time(hour=0, minute=5), self.start_new_tasks, args=(self.lucky_numbers_saver_between_callbacks,))
