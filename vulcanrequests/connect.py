...
async def create_new_connection(guild_id: int, user_id: int, school_name: str, class_name: str, group_name: str,
                                pin: str, symbol: str, token: str, channel_id: int) -> Tuple[bool, str]:
    ...
    try:
        account = await Account.register(keystore=keystore,
                                         token=token,
                                         pin=pin,
                                         symbol=symbol
                                         )
        saving = save_vulcan_data(guild_id=guild_id,
                                  user_id=user_id,
